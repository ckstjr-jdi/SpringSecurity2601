...
public class SecurityConfig {
    ...
    SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                //인증만 되어 있으면 통과 시켜줌 -> authenticated()
                        .requestMatchers("/user/**").authenticated()
                // manager/** 경로는 MANAGER 또는 ADMIN역할만 접근이 가능함. -> hasAnyRole("MANAGER", "ADMIN")
                        .requestMatchers("/manager/**").hasAnyRole("MANAGER", "ADMIN")
                // admin/** 경로는 ADMIN역할만 접근 가능함
                        .requestMatchers("/admin/**").hasAnyRole("ADMIN")
                // 위 세가지 이외의 경우에는 모든 요첨에 대해 접근 허용함(로그인 안해도 됨) -> permitAll()
                        .requestMatchers("*").permitAll())
                // 폼 로그인 기능 활성화
                // 기본 로그인 페이지(/login)를 Spring Security가 자동 제공
                // 로그인 처리 URL도 기본적으로 POST/login
                // UsernamePasswordAuthenticationFilter가 이 요청을 가로채서 인증 시도해줌
                .formLogin(Customizer.withDefaults())
                // HTTP Basic 인증 활성화
                // Postman에서 Authorization: Basic ... 헤더로 인증 가능
                .httpBasic(Customizer.withDefaults());
        return http.build();
    }
    // 메모리에 사용자 계정 3개 만드릭 설정임
    // DB연결 없어도 로그인/권한 테스트가 가능함
    @Bean
    ...
    //Spring security 5+부터는 비밀번호 저장/검증에 인코더가  없으면 에러 발생
